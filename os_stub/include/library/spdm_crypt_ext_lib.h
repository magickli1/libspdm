...
/**
 * Generate a TPM2 Quote over the selected PCRs and bind it to SPDM evidence.
 *
 * Calls libspdm_tpm_quote, then checks that extraData, PCR selection, and
 * pcrDigest match the caller-supplied nonce and PCR values. The returned
 * buffers contain TSS2-MU encoded TPM2B_ATTEST and TPMT_SIGNATURE structures.
 * The caller supplies all storage; this function never truncates.
 *
 ...
 *
 * PCR selection and pcrDigest are checked here. The signature is checked by
 * libspdm_tpm_verify_quote().
 *
