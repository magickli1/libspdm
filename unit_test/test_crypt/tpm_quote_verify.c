...
/**
 * Unit coverage for libspdm_tpm_verify_quote_evidence, which checks PCR
 * binding and then calls the committed libspdm_tpm_verify_quote helper.
 * No live TPM:
...
static bool libspdm_tpm_quote_test_make_cert(libspdm_tpm_quote_test_key_t *key,
                                             int curve_nid, const EVP_MD *digest)
{
    EVP_PKEY_CTX *pctx;
    X509 *x509;
    X509_NAME *name;
    ...
    if ((pctx == NULL) ||
        ...
        (EVP_PKEY_CTX_set_ec_paramgen_curve_nid(pctx, curve_nid) != 1) ||
    ...
    if ((x509 == NULL) || (name == NULL) ||
        ...
        (X509_sign(x509, key->pkey, digest) == 0)) {
        X509_NAME_free(name);
        X509_free(x509);
...
static bool libspdm_tpm_quote_test_sign(
    ...
    const EVP_MD *digest, TPMI_ALG_HASH tpm_hash,
    bool corrupt_signature, uint8_t *signature, size_t *signature_size)
{
    ...
    if ((md_context == NULL) ||
        (EVP_DigestSignInit(md_context, NULL, digest, NULL, pkey) != 1) ||
    ...
    tpm_sig.signature.ecdsa.hash = tpm_hash;
    ...
    bool corrupt_signature, bool expect_pass)
{
    ...
    if (!libspdm_tpm_quote_test_sign(
            ...
            EVP_sha256(), TPM2_ALG_SHA256,
            corrupt_signature, signature, &signature_size)) {
        return false;
    }

    result = libspdm_tpm_verify_quote_evidence(
...
bool libspdm_validate_crypt_tpm_quote(void)
{
    ...
    if (!libspdm_tpm_quote_test_make_cert(&key, NID_X9_62_prime256v1,
                                          EVP_sha256())) {
